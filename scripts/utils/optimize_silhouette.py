...
class CMAOptimizer():
    ...
    Attributes
    ----------
    obj_slh : pandas.DataFrame
        Geographic coordinates of the simulated image's silhouette.
        The column names must be x,y,z.
    img_slh : pandas.DataFrame
        Image silhouette of the original image.
        The column names must be u, v. See alproj.gcp.set_gcp()
    params_init : dict
        Initial values of camera parameters.
    """
    def __init__(self, obj_slh, img_slh, params_init):
        self.obj_slh = obj_slh
        self.img_slh = img_slh
        self.obj_slh
    ...
    def _loss_function(self):
        ...
        def _proj_error(values): # parameter values to optimize, as np.array
            ...
            projected = project(self.obj_slh, params)
            w = self.params_init["w"]
            h = self.params_init["h"]
            s1 = self.img_slh.values / np.array([w,h])
            s2 = projected.values / np.array([w,h])
            loss = -correlate(s1, s2, mode="same").mean()
            return loss
        return _proj_error
